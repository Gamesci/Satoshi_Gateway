...
// [MODIFIED] Added const to Template*
void stratum_send_mining_notify(int sock, const Template *tmpl) {
    ...
    json_t *m = json_array();
    // [FIXED] tmpl.merkle_count -> tmpl->merkle_count, and ->merkle_branch
            ...
            if (*start) {
                ...
                if (req) {
                    ...
                    if (!m) {
                        ...
                        json_t *params = json_object_get(req, "params");
                        (void)params; // Fix unused parameter warning
                        ...
                        // [ECO MODE] Wake up check
                        if (stratum_get_client_count() == 1) {
                             log_info("💤 -> ⚡ First miner authorized! Waking up gateway...");
