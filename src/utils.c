...
static void log_print(const char* level, const char* format, va_list args) {
    time_t now; time(&now); struct tm ts; localtime_r(&now, &ts);
    char buf[20]; strftime(buf, sizeof(buf), "%Y-%m-%d %H:%M:%S", &ts);
    fprintf(stdout, "[%s] [%s] ", buf, level); vfprintf(stdout, format, args); fprintf(stdout, "\n"); fflush(stdout);
}
void log_info(const char *format, ...) { va_list a; va_start(a, format); log_print("INFO", format, a); va_end(a); }
void log_error(const char *format, ...) { va_list a; va_start(a, format); log_print("ERROR", format, a); va_end(a); }
void log_debug(const char *format, ...) { va_list a; va_start(a, format); log_print("DEBUG", format, a); va_end(a); }

void hex2bin(const char *hex, uint8_t *bin, size_t bin_len) {
    for (size_t i = 0; i < bin_len; i++) sscanf(hex + 2*i, "%2hhx", &bin[i]);
}
...
void reverse_bytes(uint8_t *buf, size_t len) {
    for (size_t i = 0; i < len / 2; i++) { uint8_t t = buf[i]; buf[i] = buf[len-1-i]; buf[len-1-i] = t; }
}
void swap32_buffer(uint8_t *buf, size_t len) {
    if (len % 4 != 0) return;
    for (size_t i = 0; i < len; i += 4) { uint8_t t = buf[i]; buf[i] = buf[i+3]; buf[i+3] = t; t = buf[i+1]; buf[i+1] = buf[i+2]; buf[i+2] = t; }
}
uint32_t swap_uint32(uint32_t val) {
    return ((val >> 24) & 0xff) | ((val << 8) & 0xff0000) | ((val >> 8) & 0xff00) | ((val << 24) & 0xff000000);
}

// Crypto Logic (Simplified)
...
static int bech32_decode(const char* hrp, const char* addr, uint8_t *data, size_t *data_len, int *encoding) {
    ...
    p = strrchr(addr, '1'); if (!p || p == addr || p + 7 > addr + len) return 0;
    if ((size_t)(p - addr) != strlen(hrp) || strncasecmp(addr, hrp, (p - addr)) != 0) return 0;
    ...
    size_t dlen = 0;
    for (p++; *p; ++p) {
        const char *d = strchr(charset, tolower(*p)); if (!d) return 0;
        uint8_t val = (d - charset); chk = bech32_polymod_step(chk) ^ val;
...
int segwit_addr_decode(int* witver, uint8_t* witprog, size_t* witprog_len, const char* hrp, const char* addr) {
    ...
    if (data[0] > 16) return 0; if (data[0] == 0 && encoding != 1) return 0; if (data[0] > 0 && encoding != 2) return 0;
    *witver = data[0];
...
int base58_decode_check(const char *str, uint8_t *payload, size_t *payload_len) {
    uint8_t bin[128]; size_t bin_len = 0; size_t len = strlen(str); int zeros = 0;
    while (zeros < (int)len && str[zeros] == '1') zeros++;
    memset(bin, 0, sizeof(bin));
    for (const char *p = str; *p; p++) {
        const char *digit = strchr(b58digits_map, *p); if (!digit) return -1;
        int carrier = (int)(digit - b58digits_map);
        for (int i = sizeof(bin) - 1; i >= 0; i--) { int val = bin[i] * 58 + carrier; bin[i] = val & 0xFF; carrier = val >> 8; }
    }
    int i = 0; while (i < (int)sizeof(bin) && bin[i] == 0) i++;
    bin_len = zeros + (sizeof(bin) - i); if (bin_len < 4) return -1;
    if (payload) { memset(payload, 0, zeros); memcpy(payload + zeros, bin + i, bin_len - zeros); }
    uint8_t hash[32]; sha256_double(payload, bin_len - 4, hash);
    if (memcmp(hash, payload + bin_len - 4, 4) != 0) return -1;
    *payload_len = bin_len - 4; return payload[0];
}
void address_to_script(const char *addr, char *script_hex) {
    ...
    log_error("Invalid Address: %s", addr); strcpy(script_hex, "6a04deadbeef"); 
}
