#include "utils.h"
#include "sha256.h" // 确保引用了之前定义的 sha256_double
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <ctype.h>
...
static const int8_t b58digits_len = 58;
...
int base58_decode_check(const char *str, uint8_t *payload, size_t *payload_len) {
    ...
    // Convert base58 string to big integer (byte array)
    ...
    if (bin_len < 4) return -1; // Checksum requires 4 bytes
    ...
    *payload_len = bin_len - 4;
    return payload[0]; // Return version byte
}

// --- Bech32 实现 (BIP 173/350) ---
...
static int bech32_decode(const char* hrp, const char* addr, uint8_t *data, size_t *data_len, int *encoding) {
    ...
    if (strncasecmp(addr, hrp, (p - addr)) != 0) return 0; // HRP mismatch

    // HRP Expand
    for (i = 0; i < (size_t)(p - addr); ++i) chk = bech32_polymod_step(chk) ^ ((addr[i] & 0x1F) ? (addr[i] | 0x20) >> 5 : 0); // High bits
    chk = bech32_polymod_step(chk);
    for (i = 0; i < (size_t)(p - addr); ++i) chk = bech32_polymod_step(chk) ^ (addr[i] & 0x1f); // Low bits
    ...
    if (encoding) {
        if (chk == 1) *encoding = 1; // BECH32
        else if (chk == 0x2bc830a3) *encoding = 2; // BECH32M
        else return 0;
    } else {
...
int segwit_addr_decode(int* witver, uint8_t* witprog, size_t* witprog_len, const char* hrp, const char* addr) {
    ...
    if (data[0] == 0 && encoding != 1) return 0; // Version 0 must be BECH32
    if (data[0] > 0 && encoding != 2) return 0;  // Version 1+ must be BECH32M
